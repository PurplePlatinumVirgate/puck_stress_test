...
namespace Unity.Netcode
{
    ...
    public sealed class NetworkObject : MonoBehaviour
    {
        ...
        internal struct SceneObject
        {
            ...
            // PUCK_STRESS_TEST 2026-05-13 (B323 cutover): Puck's NGO
            // moved sizeOfSyncData out of SynchronizeNetworkBehaviours
            // (where vanilla NGO has it as a ushort/int prefix) and
            // into SceneObject itself as a trailing int field after
            // NetworkSceneHandle. AddSceneObject uses this for the
            // failure-path skip. Source: Puck_Beta2 decompile
            // (Unity.Netcode.Runtime/Unity/Netcode/NetworkObject.cs:2525).
            ...
            public void Deserialize(FastBufferReader reader)
            {
                ...
                // PUCK_STRESS_TEST: Puck reserves bytes for Transform (if
                // any), NetworkSceneHandle (int), AND SynchronizationDataSize
                // (int). Two ints instead of one.
                var readSize = 0;
                ...
                readSize += FastBufferWriter.GetWriteSize<int>(); // SynchronizationDataSize
                ...
                // PUCK_STRESS_TEST 2026-05-13: Puck stores the sync-block
                // length as a trailing int in SceneObject (out of
                // SynchronizeNetworkBehaviours). Used for failure-path
                // skip in AddSceneObject.
        ...
        {
            // PUCK_STRESS_TEST 2026-05-13 (B323 cutover): Puck's NGO
            // removed the in-block sizeOfSyncData prefix. The size is
            // stored in SceneObject.SynchronizationDataSize and written
            // by the caller (SceneObject.Serialize) AFTER this method
            // returns. Wire format here is just:
            //   [NV data per NB] [byte numberSynchronized]
            //                    [ushort behaviourId, Synchronize] × n
            // Source: Puck_Beta2 decompile NetworkObject.cs:2380-2417.
            if (serializer.IsWriter)
            {
                var writer = serializer.GetFastBufferWriter();

                // Synchronize NetworkVariables (no length prefix).
                foreach (var behavior in ChildNetworkBehaviours)
                {
        ...
        {
            // PUCK_STRESS_TEST 2026-05-13: Puck's NGO stores sync-block
            // length in SceneObject.SynchronizationDataSize (read during
            // Deserialize) instead of as a prefix the failure-path reads.
            // Recover the end-of-sync position up front so the failure
            // path can Seek to it without an extra read.
            ...
            if (networkObject == null)
            {
                ...
                try
                {
                    // PUCK_STRESS_TEST: Skip past the sync data using the
                    // size we already read during Deserialize.
